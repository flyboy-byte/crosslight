...
void FileBrowserActivity::showEntryActions() {
  ...
  // CrossLight: an image gets the two wallpaper actions between Open and
  // Delete. They live in this popup rather than a screen of their own because
  // upstream's entry menu is now the one place file actions are offered.
  const bool isImage = !isDirectory && FsHelpers::hasBmpExtension(entry);
  ...
  static constexpr StrId IMAGE_OPTIONS[] = {StrId::STR_OPEN, StrId::STR_SET_AS_WALLPAPER,
                                            StrId::STR_ADD_TO_WALLPAPERS, StrId::STR_DELETE, StrId::STR_RENAME};
  const StrId* options = isDirectory ? DIRECTORY_OPTIONS : (isImage ? IMAGE_OPTIONS : FILE_OPTIONS);
  const int optionCount = isDirectory ? 2 : (isImage ? 5 : 3);
  optionPopup.show(StrId::STR_FILENAME, options, optionCount, 0, [this, isImage](const int index) {
    if (isImage) {
      switch (index) {
        case 0:
          activateSelected();
          break;
        case 1:
          applyWallpaperAction(/*setAsSleepScreen=*/true);
          break;
        case 2:
          applyWallpaperAction(/*setAsSleepScreen=*/false);
          break;
        case 3:
          deleteSelected();
          break;
...
// CrossLight: set the selected image as the sleep screen, or add it to the
// rotation folder. Both may create /sleep.bmp or /.sleep, so the listing is
// reloaded afterwards.
void FileBrowserActivity::applyWallpaperAction(const bool setAsSleepScreen) {
  if (files.empty() || nav.selected < 0 || nav.selected >= listCount()) return;
  std::string cleanBasePath = basepath;
  if (cleanBasePath.back() != '/') cleanBasePath += "/";
  const std::string fullPath = cleanBasePath + files[nav.selected];

  const bool ok = setAsSleepScreen ? Wallpaper::setAsSleepScreen(fullPath) : Wallpaper::addToRotation(fullPath);
  {
    RenderLock lock(*this);
